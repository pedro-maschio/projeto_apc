...
int velocidade=60, perdeu = 0, pontuacao = 0;
double co_angular = 0;
char p; 
...
/*  Exibe o tabuleiro, rotacionando-o para que a linha fique
    posicionada corretamente, e percorrendo-o e adicionando
    as cores às letras.
*/
void exibeTabuleiro() {
    int i, j;
...
void atira() {
    int i, j, flag = 0, iant, jant;

    for(i = 0; i < largura; i++) {
        for(j = 2; j < altura-1; j++) {  // começa 2 desconta letra+borda, termina -1 desconta borda
            if(flag == 0 && tabuleiro[i][j] == '-') {
                tabuleiro[i][j] = p; // p é a letra 
                if(j > 2) {
                    tabuleiro[iant][jant] = '-';
                }
                iant = i;
